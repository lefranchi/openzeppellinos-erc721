...
contract LFRToken is ERC721PresetMinterPauserAutoId {
...
using Counters for Counters.Counter;
    ...
    constructor() ERC721PresetMinterPauserAutoId("LFRToken", "LFR", "https://lfr.art/metadata/") {}

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        require(tokenLockedFromTimestamp[tokenId] > block.timestamp || tokenUnlocked[tokenId], "LFRToken: Token locked");
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function unlockToken(bytes32 unlockHash, uint256 tokenId) public {
        require(msg.sender == ownerOf(tokenId), "LFRToken: Only the Owner can unlock the Token"); //not 100% sure about that one yet
        require(keccak256(abi.encode(unlockHash)) == tokenUnlockCodeHashes[tokenId], "LFRToken: Unlock Code Incorrect");
